        ...
        policeMapRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (DataSnapshot dsGasStation : dataSnapshot.getChildren()) {
                    PoliceItem policeItem = dsGasStation.getValue(PoliceItem.class);
                    assert policeItem != null;
                    double latPolice = policeItem.getLat_police();
                    double lngPolice = policeItem.getLng_police();
                    LatLng busPlaceLoc = new LatLng(latPolice, lngPolice);
                    policeGoogleMap.moveCamera(CameraUpdateFactory.newLatLngZoom(busPlaceLoc, 10.2f));
                    policeGoogleMap.addMarker(new MarkerOptions().position(busPlaceLoc).title(policeItem.getName_police()).snippet(policeItem.getLocation_police()));
